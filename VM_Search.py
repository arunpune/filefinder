...
# Prompt the user to enter the VM addresses to search
print("Enter the IP addresses of VMs to search, separated by commas:")
vm_addresses_input = input()
vm_addresses = [address.strip() for address in vm_addresses_input.split(',')]

for vm_address in vm_addresses:
    ssh_username = input("Enter the SSH username for {}: ".format(vm_address))
    print(f'Connecting to {vm_address}...')
    # Connect to the VM using SSH and authenticate with the public key
    ssh.connect(vm_address, username=ssh_username, pkey=key)
    print(f'Connected to {vm_address}.\n')
...
# Search for files with the selected extensions in the model directory
    print(f'Searching for model files on {vm_address}...')
    ...
    for extension in extensions:
        ...
        if len(model_files) > 0:
            print(f'The following {extension} model files were found on {vm_address}:')
            for f in model_files:
                print(f)
        else:
            print(f'No {extension} model files were found on {vm_address}.')
        
        # Search for running processes with the selected keywords
    print(f'Searching for processes on {vm_address}...')
    for keyword in process_keywords:
        ...
        if len(processes) > 0:
            print(f'The following processes were found on {vm_address} matching the keyword "{keyword}":')
            for p in processes:
                print(p)
        else:
            print(f'No processes were found on {vm_address} matching the keyword "{keyword}".')
